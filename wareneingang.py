...
async function loadOrders() {
  ...
  try {
    ...
    list.innerHTML='';
    // Lieferantennamen parallel laden
...
class Handler(BaseHTTPRequestHandler):
  ...
  <div style="background:#f9f9f9;padding:24px;border:1px solid #e0e0e0;border-radius:0 0 8px 8px">
    ...
    <p style="color:#999;font-size:12px;text-align:center">Wareneingang App · NEXTWAVE GmbH</p>
        ...
        try:
            # E-Mail via Python smtplib mit Gmail oder direktem SMTP
            import smtplib, ssl
            ...
            # Versuche über port 465 SSL
            context = ssl.create_default_context()
            with smtplib.SMTP_SSL(EMAIL_SMTP, 465, context=context) as server:
